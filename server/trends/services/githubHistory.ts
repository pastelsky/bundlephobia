...
`.trim()
  ...
  const { data } = await clickHouseClient.post<ClickHouseJson>(endpoint, sql)
      ...
      try {
        ...
        if (result.stars.length === 0) {
          ...
          for (const fallback of fallbacks) {
            const alternative = await queryClickHouse(fallback)
            if (alternative.stars.length > 0) {
              result = alternative
              break
            }
          }
...
async function fetchOssInsightStars(
  ...
  return getOrLoadTrendsData(
    ...
    async () => {
      try {
        const { data } = await ossInsightClient.get<OssInsightJson>(
          `/v1/repos/${repo}/stargazers/history/`,
          {
            params: {
              per: 'day',
              from,
  ...
  return getOrLoadTrendsData(
    ...
    async () => {
      try {
        ...
        const { data } = await githubClient.get<GithubRepoMetadata>(
          `/repos/${repo}`,
          {
            headers: {
