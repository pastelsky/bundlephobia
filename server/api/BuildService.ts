...
import { packageWorkerRegistry, pool, requestQueue } from '../init'
import { getBuildServiceEndpoints } from './buildServiceEndpoints'
    ...
    operations.forEach(operation => {
      requestQueue.addExecutor<BuildServiceJobParams, unknown>(
        ...
        async ({ packageString, onComplete }, { signal }) => {
          ...
          try {
            const buildServiceEndpoints = await packageWorkerRegistry.rank(
              packageString,
              getBuildServiceEndpoints()
            )
            if (buildServiceEndpoints.length > 0) {
              let lastTransportError: unknown
              for (const buildServiceEndpoint of buildServiceEndpoints) {
                try {
                  // Fallbacks are attempted sequentially only when a worker is unreachable.
                  ...
                  if (
                    ...
                    await packageWorkerRegistry.markUnavailable(
                      packageString,
                      buildServiceEndpoint
                    )
              ...
              this.handleError(lastTransportError, operation.type)
            }
