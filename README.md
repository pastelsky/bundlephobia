...
When the endpoint is configured but unavailable, build workers return an error
instead of starting duplicate local installations.
Remote builds are bounded to ten minutes; abandoned subscriptions expire after
fifteen minutes, while released installations remain reusable for five minutes.
