...
class TreeMap extends Component {
  render() {
    ...
    const getBorderRadius = (index) => {
      ...
      const bottomRightRadius = (Math.round(squared[index][3]) === height && Math.round(squared[index][2]) === width) ? '10px' : '0px'
...
function TreemapSquare({ children, left, top, width, height, borderRadius, data, style, ...other }) {
  return (
    <div data-vals={data.toString() + '...' + width + '...' + height} style={{
      position: "absolute",
      left,
      top,
      width,
