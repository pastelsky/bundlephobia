...
class TreemapSection extends Component {
  ...
  render() {
    ...
    depdendenciesCopy = depdendenciesCopy.map(dep => ({
      ...dep,
    ...
    if (depdendenciesCopy.length > compactLimit) {
      ...
      const percentShare = otherDependencies.reduce((acc, dep) => acc + dep.percentShare, 0)
      const sizeShare = otherDependencies.reduce((acc, dep) => acc + dep.sizeShare, 0)
              ...
              >
                {(dep.percentShare > ellipsizeLimit) ? (
                  <div>
                    ...
                    <div className="treemap__percent"
                         style={{fontSize: `${14 + Math.min(dep.percentShare * 1.2, 25)}px`}}>
                      {dep.percentShare.toFixed(1)}
                      <span className="treemap__percent-sign">
                        %
                      </span>
                    </div>
                  </div>
